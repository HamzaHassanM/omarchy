...
OpenClaw's desktop experience is its Control UI, which opens as a web app backed by its own local gateway. Omarchy installs OpenClaw into a runtime of its own under `~/.local/share/openclaw` that OpenClaw updates itself with `openclaw update`, so a new release does not wait for an Omarchy package; the `openclaw` package only carries the launcher that sets that runtime up. Removing OpenClaw under _Remove > AI_ takes the gateway service, the runtime and the app with it and then asks whether `~/.openclaw` should go too, since that holds your chats and credentials alongside the plugin runtimes OpenClaw downloads for itself; the default keeps it.
